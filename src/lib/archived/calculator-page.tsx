/**
 * ARCHIVED — Cost calculator removed from live site (Jul 2026).
 * Pricing varies by location and area; may be restored later.
 * Original route: /calculator → redirects to /contact
 */
"use client";
...
export default function ArchivedCalculatorPage() {
  ...
  const result = useMemo(() => {
    ...
    const gypsumTotal = gypsumMaterial + gypsumLabor;
  ...
  return (
    <section className="section-shell py-24">
      <h1 className="text-4xl font-bold">Cost Calculator (Archived)</h1>
        ...
        <div className="glass-card space-y-4 p-6">
          ...
          <label className="block text-sm">
            ...
            <input
              ...
              min={0}
              value={puttyCost}
              onChange={(e) => setPuttyCost(Number(e.target.value) || 0)}
              className="input-modern mt-2"
            />
          </label>
        </div>
        ...
        <div className="grid gap-4">
          <div className="glass-card p-6">
            ...
            <p className="mt-2 text-3xl font-bold text-white">
              INR {Math.round(result.cementTotal).toLocaleString("en-IN")}
            </p>
          </div>
          <div className="glass-card p-6">
            <p className="text-sm text-white/70">Gypsum plaster cost</p>
            <p className="mt-2 text-3xl font-bold text-white">
              INR {Math.round(result.gypsumTotal).toLocaleString("en-IN")}
            </p>
          </div>
          <div className="glass-card p-6">
          ...
          <div className="glass-card p-6">
            ...
            <p className="mt-2 text-3xl font-bold text-emerald-300">
              INR {Math.round(result.totalSavings).toLocaleString("en-IN")}
            </p>
          </div>
          <div className="glass-card flex flex-wrap gap-3 p-4">
            <button type="button" onClick={copyEstimate} className="button-modern inline-flex items-center gap-2">
              <Copy size={14} />
              Copy Estimate
            </button>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-emerald-500 px-4 py-2 text-xs font-semibold text-black"
            >
              <Share2 size={14} />
              Share on WhatsApp
            </button>
